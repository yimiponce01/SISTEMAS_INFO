  ...
  return (
    <aside
      className={`relative mb-6 overflow-hidden rounded-2xl border p-4 backdrop-blur-2xl transition-all duration-300 sm:p-5 ${
        darkMode ? 'bg-slate-950/72' : 'bg-slate-950/85'
      }`}
      style={{
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <section>
          ...
          <AnimalSelector selectedAnimal={selectedAnimal} setSelectedAnimal={setSelectedAnimal} />
        </section>

        <section>
          <h3 className={sectionTitleClass}>Fechas</h3>
          <DateRangeFilters dateRange={dateRange} setDateRange={setDateRange} />
        </section>

        <section>
          <h3 className={sectionTitleClass}>Acciones</h3>
          <FilterActions onClear={handleClearFilters} onApply={handleApplyFilters} />
        </section>
      </div>
    </aside>
  );
