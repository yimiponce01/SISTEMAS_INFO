...
import type { DashboardData } from '../lib/dashboardData';
    ...
    alerts.push({
        ...
        message: `Ingresos acumulados: S/ ${Number(totals?.ingresos || 0).toLocaleString()}`,
        timestamp: 'Ahora',
        color: 'green',
    });
    }
    ...
    const porcentaje =
        totals?.ingresos && totals.ingresos > 0
        ? (totals.gastos / totals.ingresos) * 100
        : 0;

    alerts.push({
        icon: AlertTriangle,
        title: porcentaje > 60 ? 'Gastos Elevados' : 'Gastos Controlados',
        message: `Los gastos representan ${porcentaje.toFixed(1)}% de los ingresos`,
        timestamp: 'Ahora',
        color: porcentaje > 60 ? 'red' : 'amber',
    });
    }
    ...
    alerts.push({
        icon: TrendingUp,
        title: 'Ganancias Positivas',
        message: `Ganancia acumulada: S/ ${Number(totals?.ganancias || 0).toLocaleString()}`,
        timestamp: 'Ahora',
        color: 'green',
    });
    }
    ...
    alerts.push({
        ...
        message: `Balance actual: S/ ${Number(totals?.balance || 0).toLocaleString()}`,
        timestamp: 'Ahora',
        color: 'green',
    });
    }
    ...
    alerts.push({
        icon: Lightbulb,
        title: 'Rentabilidad',
        message: `Rentabilidad actual: ${(totals?.rentabilidad ?? 0).toFixed(2)}%`,        timestamp: 'Ahora',
        color: (totals?.rentabilidad || 0) > 50 ? 'green' : 'amber',
    });
    }
    ...
    alerts.push({
        ...
        message: `Se registran pérdidas por S/ ${Math.abs(totals?.ganancias || 0).toLocaleString()}`,
        timestamp: 'Ahora',
        color: 'red',
    });
    }
